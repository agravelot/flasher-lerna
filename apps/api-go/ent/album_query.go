...
// QueryAlbumCategories chains the current query on the "album_categories" edge.
func (aq *AlbumQuery) QueryAlbumCategories() *AlbumCategoryQuery {
	query := &AlbumCategoryQuery{config: aq.config}
...
func (aq *AlbumQuery) QueryAlbumCosplayers() *AlbumCosplayerQuery {
	query := &AlbumCosplayerQuery{config: aq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := aq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (aq *AlbumQuery) FirstID(ctx context.Context) (id int32, err error) {
	var ids []int32
...
func (aq *AlbumQuery) FirstIDX(ctx context.Context) int32 {
...
func (aq *AlbumQuery) OnlyID(ctx context.Context) (id int32, err error) {
	var ids []int32
...
func (aq *AlbumQuery) OnlyIDX(ctx context.Context) int32 {
...
func (aq *AlbumQuery) IDs(ctx context.Context) ([]int32, error) {
	var ids []int32
...
func (aq *AlbumQuery) IDsX(ctx context.Context) []int32 {
	ids, err := aq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
// WithAlbumCategories tells the query-builder to eager-load the nodes that are connected to
// the "album_categories" edge. The optional arguments are used to configure the query builder of the edge.
func (aq *AlbumQuery) WithAlbumCategories(opts ...func(*AlbumCategoryQuery)) *AlbumQuery {
	query := &AlbumCategoryQuery{config: aq.config}
	for _, opt := range opts {
		opt(query)
	}
	aq.withAlbumCategories = query
	return aq
}
...
func (aq *AlbumQuery) WithAlbumCosplayers(opts ...func(*AlbumCosplayerQuery)) *AlbumQuery {
	query := &AlbumCosplayerQuery{config: aq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (aq *AlbumQuery) sqlAll(ctx context.Context) ([]*Album, error) {
	...
	if query := aq.withAlbumCategories; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int32]*Album)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.AlbumCategories = []*AlbumCategory{}
		}
		query.Where(predicate.AlbumCategory(func(s *sql.Selector) {
			s.Where(sql.InValues(album.AlbumCategoriesColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			...
			node.Edges.AlbumCategories = append(node.Edges.AlbumCategories, n)
		}
	}

	if query := aq.withAlbumCosplayers; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int32]*Album)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
		...
		query.Where(predicate.AlbumCosplayer(func(s *sql.Selector) {
			s.Where(sql.InValues(album.AlbumCosplayersColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
